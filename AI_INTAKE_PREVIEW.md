...
AI 预览需求用于承接外部识别流程已经写入数据库的候选需求。系统先在管理端展示候选需求和证据链，管理员或对应业务负责人点击单条候选后自动填充左侧手动录入表单，可修改字段，确认后才正式写入 `requirements`、`requirement_items` 和 `tasks`。
...
1. 页面加载后调用 `GET /api/v1/requirements/ai-preview-candidates?limit=12&scope=mine`。
2. 右侧 `AI预览需求` 面板按一条候选需求一张横向卡片展示。
3. 管理员可在面板顶部切换 `我的待确认`、负责人筛选下拉框和 `历史真实需求`；负责人下拉框包含 `全部待确认` 与各业务组负责人。
4. 管理员选择某个负责人时，请求 `scope=all&reviewOwnerId=<ownerUserId>`，只显示该负责人名下待确认候选；普通负责人仍只显示自己的待确认候选。
5. 卡片左侧显示候选摘要：群聊/客户、业务平台、业务分类、时间、置信度和状态。
6. 卡片右侧显示证据链：消息时间、发送人、消息文本和证据原因。
7. 点击卡片后，左侧切换到手动录入并自动填充字段。
8. 有确认权限的用户可修改客户、平台、分类、时间、标题、内容等字段。
9. 点击 `确认需求` 后，复用现有 `POST /api/v1/requirements/with-task` 创建正式需求并生成待指派任务。
10. 创建成功后调用 `POST /api/v1/requirements/ai-preview-candidates/{candidateId}/confirm`，将候选状态标记为 `confirmed`。
11. 已确认候选不再出现在 AI 预览待确认列表中，可在 `历史真实需求` 查看。
...
- `scope`：可选，`mine` 表示当前负责人待确认，`all` 表示全部待确认，`processed` 表示历史真实需求；非管理员强制为 `mine`。
- `reviewOwnerId`：可选，仅管理员在 `scope=all` 时生效，用于按负责人过滤待确认候选。
...
- 管理员可确认全量候选；业务负责人只可确认自己负责范围内的候选；执行人不可确认。
...
```bash
GET /api/v1/requirements/ai-preview-candidates?limit=3&scope=mine
GET /api/v1/requirements/ai-preview-candidates?limit=100&scope=all&reviewOwnerId=<ownerUserId>
POST /api/v1/requirements/ai-preview-candidates/{candidateId}/confirm
```
