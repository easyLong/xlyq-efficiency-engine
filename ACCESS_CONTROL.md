...
| --- | --- | --- | --- |
| 需求与指派 | 可见 | 可见 | 可见 |
| AI 预览需求 | 全量可见并可确认；可按业务负责人筛选全部待确认候选 | 只看负责范围并可确认 | 不可见 |
...
## 2026-07-02 AI 预览筛选补充

- 管理员拥有 `*` 或 `ai_preview.view_all` 权限时，AI 预览面板可使用负责人下拉框筛选 `全部待确认` 候选。
- 负责人筛选调用 `GET /api/v1/requirements/ai-preview-candidates?scope=all&reviewOwnerId=<ownerUserId>`。
- 后端按权限点判断是否允许全量和负责人筛选；业务负责人即使传入 `scope=all` 或 `reviewOwnerId`，也会被强制收敛为 `mine`。
- 普通执行人没有 `ai_preview.view_owned` / `ai_preview.view_all`，不显示 AI 预览面板。
...
- 业务负责人和执行人默认没有 `page.group_management`，不会显示群管理入口。
