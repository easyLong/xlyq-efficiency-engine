...
- 员工本地交付登记页：`GET /asset-sheet.html?taskId=<taskId>&taskNo=<taskNo>&token=<token>`，支持服务器草稿和正式提交两个独立动作；页面顶部可返回执行人的个人任务视图。
- 审核资产页：`GET /asset-review.html?taskId=<taskId>&token=<token>`，一审、二审候选人可从飞书卡片直达；多人审核必须先领取工作项，其他候选人随即转为只读。
- 当前默认数据库：`ops_platform`
- 登录与接口：管理后台使用员工姓名下拉选择 + 密码登录，下拉账号使用 `display_name`，账号是否可登录由 `users.login_enabled` 和角色配置决定。飞书通知链接携带任务 token 与身份交接参数直达对应任务；受保护接口使用 `Authorization: Bearer <accessToken>`，登录结果返回角色、权限点和数据范围。
...
对接人/基金/平台/业务分类
  -> 录入需求并自动生成任务
  -> 指派员工并发送飞书消息
  -> 员工登记图片资产和合作链接，可保存服务器草稿
  -> 正式提交后按业务大类一审、按基金二审
  -> 导入合同报价并生成报价子项
  -> 历史需求任务中选择报价子项
  -> 按资产个数 × 已确认报价单价生成结算统计，并可导出 Excel / 资产 PPT
  -> 需求面板按基金、平台、分类和时间下钻，人员排行独立统计
  -> 派发者查看全局员工负载、员工详情，推荐空闲执行人并催促交付
```
...
- 客户：需求录入下拉显示全部 active 客户，系统默认补入内部项目客户“向量引擎”，默认对接人为“雷声”；报价、映射和结算仍以基金客户为主要范围。
- 对接人
...
需求面板中，基金、平台、业务大类、二级分类属于联动下钻维度；执行员工和派发人作为独立排行维度，只受起止时间影响，不跟随饼图下钻。

派发者可在需求面板查看全局员工负载与可用度，但该看板是只读调度视图；实际需求创建、任务编辑和任务指派仍按派发者负责基金范围校验。员工历史任务统计和派发人任务看板会显示同步的飞书头像，无头像时显示姓名首字。

对接人配置当前只明确维护基金客户和业务平台；业务大类与二级分类由独立关系表维护，大类选定后二级分类下拉联动。三级分类已不再作为全局筛选维度，但历史字段仍保留用于兼容旧数据。

合同报价录入本身只选择基金客户、合同起止月份和报价文件；报价合同内可以覆盖多个业务大类，同一家基金允许按不同期间维护多份合同。合同展示名采用“基金名称合同报价(yyyyMM~yyyyMM)”，系统编号保留为副标题。CSV 合同优先按表格结构解析，识别最细粒度子项、单位、单价，并把最后层级和备注放入子项详情。
...
- 有效流程角色统一为管理员、派发者、执行人、一审人员和二审人员；旧 `owner` 角色退出数据范围和流程判断。
- `tasks.created_by_user_id` 只记录创建审计人，`dispatcher_user_id` 记录实际派发人，`assignee_user_id` 记录执行人；`reporter_user_id` 仅保留历史查看链接兼容。
- 一审团队按业务大类配置，二审团队按基金配置；权限画像统一返回 `second_reviewer`，同时兼容旧数据库角色值 `customer_owner`。
- 一审、二审采用工作项候选人模型。多人收到同一审核任务后必须先领取，数据库原子更新保证只有一人可以处理。
- 资产登记页增加服务器草稿：保存草稿不改变任务状态、不发审核通知、不写正式资产统计；正式提交才写入 `task_result_files` 并进入待一审。
- 当前回归结果为 14 个测试套件、44 项测试全部通过，Nest 构建、页面脚本和数据库健康检查均正常。

以下按日期保留的更新记录用于追溯历史实现；其中早期“负责人直接验收”描述不再代表当前流程口径。

## 2026-06-26 交付与结算导出更新

- 员工资产登记页顶部新增醒目的 `个人任务主页` 按钮，点击后只进入执行人临时视图和历史任务列表，不保存资产、不提交交付、不改变任务状态。
- 本地交付登记字段统一为“合作链接”，可记录多条最终合作交付页面地址；合作链接不计入结算资产个数。
- AI 预览需求“标记伪需求”改为收集原因、补充说明和是否进入提示词优化样本，后端写入 `demand_candidate_review_logs` 并同步回写候选需求复核备注。
- 结算统计明细和 Excel 导出补充“对接人”字段，列表标题区显示清洁的对接人名称，不再展示拼接噪声。
- 资产 PPT 导出改为按已确认报价子项层级排版，标题为“基金名称-结算项目”，正文只保留层级标题和图片资产；图片使用原始文件数据嵌入，图片下方显示真实文件名，例如 `1 播放页banner`。
...
- 历史需求任务状态优化了指派、改派、报价选择、报价确认后的即时刷新；报价选择改为先选合同，再联动展示该合同下的报价子项。
...
- 当前管理员：雷声、韦莉香、廖丽婷。
## 2026-07-01 群管理与合同报价数据更新

- 新增“群管理”页面，支持维护微信群、基金客户、业务平台、群昵称、对接人和微信备注进度。
- 群新增流程统一为：拉 DP 入群 -> 新增群信息 -> DP 微信号备注客户真名。
- `group_contact_mappings` 新增 `group_nickname` 和 `nickname_updated` 字段，后端启动时会安全补齐历史表结构。
- 群管理列表按微信群名称聚合为“一群一行”，标题显示“X 个群已经加入监控”；底层仍保持“一个群 + 一个对接人 = 一行”，便于需求录入精确选择对接人。
- 新增群支持在 `contactName` 和 `groupNickname` 中用中英文逗号、顿号、分号或 `|` 配置多个对接人/昵称，系统会自动去除每项前后空格并拆成多行保存。
- 合同报价列表优先使用关联项目名称展示，便于识别“某某基金2026年合同报价”。
- 批量补录 2026 年度基金合同报价，并统一报价子项编码为 `ITEM-001` 这种短格式。
